...
<template>
  <aside class="h-screen lg:sticky lg:top-0 w-64 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 flex flex-col">
    <div class="h-16 shrink-0 flex items-center justify-between px-4 gap-3 border-b border-gray-100 dark:border-gray-700">
    ...
    <nav class="flex-1 overflow-y-auto px-2 py-3">
      <div v-for="group in menuGroups" :key="group.title" class="mb-3">
        <button 
          class="w-full flex items-center justify-between px-3 py-1.5 text-xs font-semibold text-gray-500 uppercase tracking-wider hover:text-gray-600"
        ...
        <Transition name="slide">
          <div v-show="expandedGroups[group.title]" class="space-y-0.5 mt-1">
            <RouterLink
              ...
              :to="item.to || '/'"
              class="flex items-center gap-3 px-3 py-2.5 min-h-11 rounded-lg text-sm transition-colors duration-150"
